...
class GitHubRepositoryManager:
    ...
    def update_existing_repo(self, repo_info):
        ...
        try:
            ...
            # Clonează repository-ul existent și face update (include commit/push)
            result = self.clone_and_update_repo(repo_info)
            if result:
                return repo_info  # Returnează repo_info pentru success message
            else:
                return None
    ...
    def clone_and_update_repo(self, repo_info):
        ...
        try:
            ...
            # Actualizează cu fișierele noi (rămânem în directorul git)
            ...
            # Commit și push (încă în directorul git)
            commit_result = self.commit_and_push(repo_info, is_update=True)

            # Acum ieșim din directorul git
            os.chdir(original_dir)
    ...
    def prepare_new_repo(self, repo_info):
        ...
        try:
            ...
            # Commit și push (încă în directorul git)
            commit_result = self.commit_and_push(repo_info, is_update=False)
    ...
    def run(self):
        ...
        try:
            ...
            if existing_repo:
                ...
                # Pentru update, commit și push sunt deja făcute în clone_and_update_repo
                upload_success = True  # Presupunem că s-a făcut deja
            else:
                ...
                # Pregătește repository nou (include și commit/push)
                upload_success = self.prepare_new_repo(repo_info)
...
def main():
    ...
    print("="*70)
    print("🏭 MANUFACTURING PROJECT → GITHUB REPOSITORY MANAGER V2.2")
    print("="*70)
    ...
    print("✅ Fixed Git directory navigation issues")
    print("✅ Proper commit/push workflow")
    ...
    print("🐛 Major fix: Git operations now work in correct directory")
